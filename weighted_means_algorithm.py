                ...
                for k in xrange(len(groups[i])):
                    if checkVar == groups[i][k]:
                        if vals[k] is not None:
                            ...
                            if k == len(groups[i]) - 1:
                                resultList.append(dic) # Adds dic to resultList before dic gets reinitialized to empty on next iteration
        keepTrackList = []
        switch = True
        #print resultList # What the list looks like
    find_mean(vals, groups, weights, resultList)

# Step 1B (Find Mean Values)
def find_mean(vals, groups, weights, resultList):
    """
    Finding The Mean Value

    The code iterates through resultList and accesses the first key which is either
    the string 'Country', 'State', or 'City'.
    This is achieved by resultList[i].items)[0][0].
    To find the mean we must destructure resultList and access each elements value 
    and divide it by the number of times it occurs.
    Example:
        resultList[0] --> Accesses the first element of resultList
        resultList[0][countryOrStateorCity] --> Destructures the list and accesses the Country dictionary
        resultList[0][countryOrStateorCity][countryName]['Value'] --> Continues destructuring all the way down 
    ...
    keepTrackList = []
    for i in xrange(len(groups)):
        countryOrStateOrCity = resultList[i].items()[0][0]
        ...
        if countryOrStateOrCity == 'Country':
            for x in xrange(len(groups[0])):
                ...
                if countryName not in keepTrackList:
                    #print countryName
                    if resultList[0][countryOrStateOrCity][countryName]['Value'] is None:
                        countryMean[countryName] = None
                        keepTrackList.append(countryName)
                    else:
                        #print resultList[0][countryOrStateOrCity][countryName]['Value']
                        value = float(resultList[0][countryOrStateOrCity][countryName]['Value'])
                        occurences = float(resultList[0][countryOrStateOrCity][countryName]['Occurences'])
            ...
            for x in xrange(len(groups[1])):
                ...
                if stateName not in keepTrackList:
                    #print stateName
                    if resultList[1][countryOrStateOrCity][stateName]['Value'] is None:
                        stateMean[stateName] = None
                        keepTrackList.append(stateName)
                    else:
                        #print resultList[0][countryOrStateOrCity][countryName]['Value']
                        value = float(resultList[1][countryOrStateOrCity][stateName]['Value'])
                        occurences = float(resultList[1][countryOrStateOrCity][stateName]['Occurences'])
                        mean = value / occurences
            ...
            for x in xrange(len(groups[2])):
                ...
                if cityName not in keepTrackList:
                    #print cityName
                    if resultList[2][countryOrStateOrCity][cityName]['Value'] is None:
                        cityMean[cityName] = None
                        keepTrackList.append(cityName)
                    else:
                        #print resultList[0][countryOrStateOrCity][countryName]['Value']
                        value = float(resultList[2][countryOrStateOrCity][cityName]['Value'])
                        occurences = float(resultList[2][countryOrStateOrCity][cityName]['Occurences'])
                        mean = value / occurences
